...
def _active_table(conn=None) -> str:
    """The position table for the current mode — positions in real,
    paper_positions in paper. post_mortem operates on whichever is live."""
    try:
        from trading import config as fa_config
        return "positions" if fa_config.is_real_mode() else "paper_positions"
    except Exception:
        return "positions"


def _ensure_table(conn):
    """Idempotent — adds the postmortem columns to BOTH positions and
    paper_positions if missing (so 1:1 parity holds across modes), and
    creates the helper trade_postmortem table.
    """
    for tbl in ("positions", "paper_positions"):
        try:
        ...
        if "postmortem_tag" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_tag TEXT")
        if "postmortem_severity" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_severity TEXT")
        if "postmortem_reason" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_reason TEXT")
        if "postmortem_evidence" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_evidence TEXT")
        if "postmortem_done" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_done INTEGER DEFAULT 0")
        if "postmortem_cost_usd" not in cols:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN postmortem_cost_usd REAL")
    conn.commit()
...
def _candidates(conn, lookback_hours: int = 48) -> list[dict]:
    ...
    1:1 across modes — queries `positions` in real mode and `paper_positions`
    in paper mode. Field shape returned is identical.
    """
    tbl = _active_table(conn)
    if tbl == "positions":
        rows = conn.execute(
        ...
        # paper_positions: closed_at + status='closed', no chain/setup_type
        # columns; alias them so caller sees identical shape.
        rows = conn.execute(
            "SELECT id, symbol, direction, entry_price, "
            "       NULL AS close_price, tp_levels, "
            "       realized_pnl, close_reason, archetype_at_open, "
            "       archetype AS setup_type, "
            "       ai_score_at_open, bear_phase_at_open, "
            "       closed_at AS close_time, "
            "       leverage, opened_at AS open_time "
            "FROM paper_positions "
            "WHERE status='closed' AND (is_hedge IS NULL OR is_hedge=0) "
            "AND closed_at IS NOT NULL AND closed_at != '' "
            "AND (postmortem_done IS NULL OR postmortem_done=0) "
            "AND realized_pnl IS NOT NULL "
            f"AND closed_at >= datetime('now', '-{int(lookback_hours)} hours') "
...
def _persist_postmortem(conn, trade: dict, parsed: dict,
    ...
    tbl = _active_table(conn)
    conn.execute(
        f"UPDATE {tbl} SET postmortem_tag=?, postmortem_severity=?, "
...
def top_recurring_tags(conn, window_days: int = 7, limit: int = 3) -> list[dict]:
    ...
    if tbl == "positions":
        ...
        where = ("status='closed' AND (is_hedge IS NULL OR is_hedge=0) "
                 "AND postmortem_done=1 AND postmortem_tag IS NOT NULL "
                 f"AND closed_at >= datetime('now', '-{int(window_days)} days')")
    rows = conn.execute(
